...
use super::catalog::{AgentCandidate, AgentLaunch, bundled_registry};
...
fn install_registry_binary(runtime_dir: &Path, registry_id: &str) -> Result<PreparedProgram> {
    ...
    let registry = bundled_registry().context("failed to read bundled ACP registry")?;
    let agent = registry
        .agents
        .into_iter()
    ...
    let binary = agent
        .distribution
        .binary
        .and_then(|mut m| m.remove(platform))
    ...
    let version_dir = runtime_dir.join("versions").join(&agent.version);
    let install_root = runtime_dir
        .join("install")
        .join(format!("{registry_id}-{}", agent.version));
...
pub(super) fn prepare_agent_command(candidate: AgentCandidate) -> Result<Vec<String>> {
    ...
    match &candidate.launch {
        AgentLaunch::Binary { registry_id } => {
            prepare_native_command(&candidate, user_cli, registry_id)
        }
...
mod tests {
    ...
    use super::{cached_managed_binary, safe_join};
    ...
    fn loads_cached_managed_binary_manifest() {
        ...
        let cached = cached_managed_binary(&runtime_dir).expect("find cached binary");
