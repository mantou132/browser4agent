...
fn get_native_message_dir(browser: Browser) -> Option<PathBuf> {
    ...
    if cfg!(target_os = "macos") {
        let relative = match browser {
            ...
            Browser::Chrome | Browser::ChromeCanary => {
                &format!("Library/Application Support/Google/{}/NativeMessagingHosts", browser)
            }
            _ => {
                &format!("Library/Application Support/{}/NativeMessagingHosts", browser)
            }
        };
        ...
        let relative = match browser {
            ...
            Browser::Chrome | Browser::ChromeCanary => {
                &format!(".config/google-{}/NativeMessagingHosts", browser.to_string().to_lowercase().replace(' ', "-"))
            }
            _ => {
                &format!(".config/{}/NativeMessagingHosts", browser.to_string().to_lowercase().replace(' ', "-"))
            }
        };
        Some(home.join(relative))
    } else if cfg!(target_os = "windows") {
        let app_data = dirs::data_local_dir()?;
        Some(app_data.join("NativeMessagingHosts").join(if browser.is_firefox() { "Firefox" } else { "Chrome" }))
    } else {
        None
    }
}
...
fn write_registry(name: &str, browser: Browser, manifest_path: &Path) -> Result<()> {
    ...
    println!("Wrote registry key: HKCU\\{} = {}", key_path, manifest_path.display());
    Ok(())
}
...
pub fn install_native_message_host(
    ...
    for browser in Browser::iter() {
        ...
        if let Some((key, value)) = if browser.is_firefox() {
            ...
            chrome_extension_id.map(|id| ("allowed_origins", json!([format!("chrome-extension://{}/", id)])))
        } {
            manifest[key] = value;
        }
