      ...
      <section class="my-2 w-full rounded-lg border border-border bg-bg-light/60">
        <header class="px-3 py-2">
          <div class="flex items-center justify-between gap-3">
            ...
            <span v-if=${toolCall.kind} class="shrink-0 font-mono text-xs text-describe">${toolCall.kind}</span>
          </div>
          <p
            class="mb-0 mt-0.5 truncate font-mono text-xs text-describe"
        ...
        <footer class="flex flex-wrap justify-end gap-2 border-t border-border px-3 py-2">
          <dy-button small color="cancel" @click=${() => this.#choose(null)}>
